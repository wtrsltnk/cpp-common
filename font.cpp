...
Font::Font()
    : _texture(0), cdata(nullptr), startchar(0), endchar(128), _fontsize(12.0f)
{ }
...
bool Font::init(const std::string & fontname, float size, int startchar, int endchar)
{
    ...
    if (io != nullptr)
    {
        ...
        {
            if (this->cdata != nullptr) delete []this->cdata;
            this->cdata = new stbtt_bakedchar[this->endchar - this->startchar];
            auto bitmap = new unsigned char[imagesize * imagesize];
            stbtt_BakeFontBitmap(buffer, 0, this->_fontsize, bitmap, imagesize, imagesize, this->startchar, this->endchar - this->startchar, this->cdata);
